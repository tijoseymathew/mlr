...
makeRLearnerInternal = function(id, type, package, par.set, par.vals, properties,
  ...
  assertString(id)
  assertChoice(type, choices = c("classif", "regr", "multilabel", "surv", "cluster", "costsens", "phmregr"))
...
#' @export
#' @rdname RLearner
makeRLearnerPHMRegr = function(cl, package, par.set, par.vals = list(), properties = character(0L), name = cl, short.name = cl, note = "", callees = character(0L)) {
  addClasses(
    makeRLearnerInternal(cl, "phmregr", package, par.set, par.vals, properties, name, short.name, note, callees),
    c(cl, "RLearnerPHMRegr")
  )
}
